...
var newUserInterval = "";
function haveTeamPower(){
	$.post("/system/user/haveTeamPower",{},function(res){
    	if(res.code == 0){
    		checkNewUser();
    		newUserInterval = setInterval(function(){
    			checkNewUser();
    		},30000);
        }
    })
...
function checkNewUser(){
	$.post("/system/user/checkNewUser",{},function(res){
    	if(res.code == 0){
    		let time = 2*60*1000;
    		layer.open({
		        title: "新用户通知",
...
//		        shadeClose: true,
		        time : time,
		        area: ['auto', 'auto'],
		        content: '<h2>有新用户，请及时处理</h2>'
		    });
    		$("#newnotify").html('<audio src="/radio/newuser.mp3" autoplay="autoplay" style="display:none"  >');
...
$(function() {
    ...
    function setActiveTab(element) {
        if (!$(element).hasClass('active')) {
            ...
            // 显示tab对应的内容区
            $('.hs_iframe').each(function() {
                if ($(this).data('id') == currentId) {
                    $(this).show().siblings('.hs_iframe').hide();
                }
            });
    ...
    function menuItem() {
        ...
        $('.menuTab').each(function() {
            if ($(this).data('id') == dataUrl) {
                if (!$(this).hasClass('active')) {
                    ...
                    scrollToTab(this);
                    // 显示tab对应的内容区
                    $('.mainContent .hs_iframe').each(function() {
                        if ($(this).data('id') == dataUrl) {
                            $(this).show().siblings('.hs_iframe').hide();
                            return false;
                        }
        ...
        if (flag) {
            ...
            // 添加选项卡对应的iframe
            var str1 = '<iframe class="hs_iframe" name="iframe' + dataIndex + '" width="100%" height="100%" src="' + dataUrl + '" frameborder="0" data-id="' + dataUrl + '" seamless></iframe>';
            $('.mainContent').find('iframe.hs_iframe').hide().parents('.mainContent').append(str1);
    ...
    function refreshTab() {
    	...
    	var target = $('.hs_iframe[data-id="' + currentId + '"]');
    ...
    function tabCloseOther() {
        $('.page-tabs-content').children("[data-id]").not(":first").not(".active").each(function() {
            $('.hs_iframe[data-id="' + $(this).data('id') + '"]').remove();
            $(this).remove();
        });
    ...
    function tabCloseAll() {
        ...
        $('.page-tabs-content').children("[data-id]:first").each(function() {
            $('.hs_iframe[data-id="' + $(this).data('id') + '"]').show();
    ...
    $.contextMenu({
        ...
        items: {
            ...
            "full": {
                ...
                callback: function(key, opt) {
                	setActiveTab(this);
                    var target = $('.hs_iframe[data-id="' + this.data('id') + '"]');
