...
(function ($) {
    $.extend({
        ...
        operate: {
            ...
            // 打开页面
            open: function(id) {
                $.modal.open($.table._option.openName, $.operate.openUrl(id));
            },
            openTab: function(id) {
            	$.modal.openTab($.table._option.openName, $.operate.openUrl(id));
            },
            // 添加信息
            add: function(id) {
            ...
            // 打开页面反问地址
            openUrl: function(id) {
            	var url = $.common.isEmpty(id) ? $.table._option.openUrl.replace("{id}", "") : $.table._option.openUrl.replace("{id}", id);
            	return url;
            },
            // 修改信息
            edit: function(id) {
            ...
            successTabCallback: function(result) {
                if (result.code == web_status.SUCCESS) {
    	            ...
    	            var $contentWindow = $('.hs_iframe[data-id="' + currentId + '"]', topWindow)[0].contentWindow;
    	            $.modal.close();
        ...
        tree: {
        	...
        	init: function(options) {
        	    ...
        	    $.get(options.url, function(data) {
        			...
        			if(options.showNum){
        				var paramLists = tree.getNodes();
        				$.tree.getNum(paramLists);
        			}
        		});
        	},
        	getNum: function(paramLists){
    		...
    		　　　　$._tree.updateNode(paramLists[i]);
    		　　　　if(num>0){
    		　　　　　　$.tree.getNum(paramLists[i].children)
    		　　　　};
    		　　}
        	},
        	// 搜索节点
        	...
        	notAllowParents: function(_tree) {
    		    ...
    		    console.log(_tree.getNodes().length);
    		    if(_tree.getNodes().length > 1)
    		    for (var i = 0; i < nodes.length; i++) {
    		        if (nodes[i].level == 0) {
