...
public class IPSeeker {
	...
	public static String COUNTRY_AREA = "|";
	/**
	...
	private static final String IP_FILE = Global.getQQwryPath() + File.separator + "qqwry.dat";
	...
	private IPSeeker() {
		...
		try {
			...
			logger.error("IP地址信息文件没有找到，IP显示功能将无法使用----" + IP_FILE);
			ipFile = null;
	...
	public List<IPEntry> getIPEntries(String s) {
		...
		try {
			...
			logger.error("ip地址解析错误--》"+e.getMessage(),e);
		}
		return ret;
	}
	...
	private void readIP(long offset, byte[] ip) {
		try {
			...
			logger.error("ip地址读取错误--》" + e.getMessage(),e);
		}
	}
	...
	private String readString(long offset) {
		try {
			...
			logger.error("readString--ip地址读取错误--》"+e.getMessage());
		}
		return "";
	}
	...
	private String readString(int offset) {
		try {
			...
			logger.error("ip地址读取错误---》"+e.getMessage());
		}
		return "";
	}
	...
	public String getAddress(String ip) {
		...
		String address = country + COUNTRY_AREA + area;
		return address.trim();
	}
...
//		System.out.println(IPSeeker.class.getResource("/qqwry.dat").toString());
		...
		String ip = "210.195.201.12";
		System.out.println(is.getCountry(ip));
//		System.out.println(is.getArea(ip));
//		System.out.println(is.getAddress(ip));
	}
